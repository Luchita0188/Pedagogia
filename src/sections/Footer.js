...
class Footer extends Component {
    ...
    if (actualIndex !== 0) {
      return(
        <div className = 'buttonPannel'>
          <button
            className = { 'buttonNav ' + (actualIndex === 1 || actualIndex === 10 ? 'disabled': '') }
            id = 'btnNavLeft'
            ...
            <button
            className = { 'buttonNav ' + (actualIndex === limitNavigation || Object.values(data)[actualIndex].endCourse === true ? 'disabled': '') + (actualIndex === 1 || actualIndex === 9 || actualIndex === 10 || actualIndex === 11 ? 'disabled': '') }
            id = 'btnNavRight'
            onClick = { this.navigationCourse }>
            <FontAwesomeIcon
              className = 'iconButton'
  ...
  showPage = () => {
    ...
    if (actualIndex !== 0) {
      return(
        <h2 className = 'numberPage fw-7 azul'>{ ((actualIndex) <= 9 ? '0' + (actualIndex) : (actualIndex)) }/11</h2>
      );
    }
  }
  ...
  render() {
    ...
    return (
      <div className = { (actualIndex === 0 || actualIndex === 11 ? 'footer-desc' : 'footer') + ' d-Flex j-S aI-C' }>
